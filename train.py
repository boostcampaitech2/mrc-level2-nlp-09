...
import dataclasses
from datasets import load_metric, load_from_disk, Dataset, DatasetDict, Features, Sequence, Value, concatenate_datasets, load_dataset
...
def run_mrc(
    ...
    def prepare_train_features_qg(examples):
        # truncation과 padding(length가 짧을때만)을 통해 toknization을 진행하며, stride를 이용하여 overflow를 유지합니다.
        # 각 example들은 이전의 context와 조금씩 겹치게됩니다.
        tokenized_examples = tokenizer(
            examples[question_column_name if pad_on_right else context_column_name],
    ...
    if training_args.do_train:
        ...
        # question generation 결과로 dataset 생성
        # qg_dataset_df = pd.read_pickle('QG_utils/qg.pkl')
        # qg_dataset = Dataset.from_pandas(qg_dataset_df)
        qg_dataset = load_from_disk('QG_utils/qg_origin_dataset')
        train_dataset_qg = qg_dataset.map(
            prepare_train_features,
            batched=True,
            num_proc=data_args.preprocessing_num_workers,
            remove_columns=['context', 'question', 'answers', 'document_id'],
            load_from_cache_file=not data_args.overwrite_cache,
        )
